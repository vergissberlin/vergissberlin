...
- The social icons in the README are pinned to `simple-icons@v3` on jsDelivr.
  ...
  bumping the pin.
- The cards in the *GitHub activity* section are not generated by any workflow
  here. They are rendered on request by `github-profile-summary-cards.vercel.app`
  and `github-profile-trophy.vercel.app` — no marker block, no commit, but also
  no control over those hosts. They replaced
  `github-readme-stats.vercel.app`, whose shared instance is rate-limited to the
  point of mostly serving "Maximum retries exceeded".
  If these hosts become unreliable as well, the five summary cards can be
  pre-generated instead: the upstream Action
  [`vn7n24fzkq/github-profile-summary-cards`](https://github.com/vn7n24fzkq/github-profile-summary-cards)
  commits them as SVGs into a branch of this repository (`AUTO_PUSH`,
  `BRANCH_NAME`), and the README embeds those files from
  raw.githubusercontent.com. That trades a third-party render at page load for
  one more scheduled workflow with `contents: write`. The trophy image has no
  such option.
